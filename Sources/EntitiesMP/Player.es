...
extern FLOAT hud_fScaling     = 0.5f;
...
extern FLOAT gfx_fEnvParticlesDensity = 0.0f;
extern FLOAT gfx_fEnvParticlesRange = 0.0f;
//extern INDEX gfx_bRenderFog = FALSE;
...
void CPlayer_OnInitClass(void)
{
  ...
  _pShell->DeclareSymbol("INDEX cht_bDFB;", &cht_bDFB);
  _pShell->DeclareSymbol("INDEX cht_bDFBD;", &cht_bDFBD);
  _pShell->DeclareSymbol("INDEX cht_bDFBA;", &cht_bDFBA);
  ...
  // mod chit
  _pShell->DeclareSymbol("persistent user INDEX dwk_bShowExtraStats;",    &dwk_bShowExtraStats);
  _pShell->DeclareSymbol("persistent user INDEX dwk_bShowEnemyCount;",    &dwk_bShowEnemyCount);
  _pShell->DeclareSymbol("persistent user INDEX dwk_bShowPlayerCoords;",  &dwk_bShowPlayerCoords);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowEnemyRadar;",    &hud_bShowEnemyRadar);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowPlayerRadar;",   &hud_bShowPlayerRadar);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowPlayerRadar2;",  &hud_bShowPlayerRadar2);
  _pShell->DeclareSymbol("persistent user INDEX hud_iCrosshairColor;",    &hud_iCrosshairColor);
  _pShell->DeclareSymbol("persistent user INDEX hud_bColorCritterRadar;", &hud_bColorCritterRadar);
  _pShell->DeclareSymbol("persistent user FLOAT hud_fFOV;",               &hud_fFOV);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowScore;",         &hud_bShowScore);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowPEInfo;",        &hud_bShowPEInfo);
	//_pShell->DeclareSymbol("persistent user INDEX dwk_bBotView;",           &dwk_bBotView);
  //_pShell->DeclareSymbol("persistent user INDEX dwk_iBotView;",           &dwk_iBotView);
  _pShell->DeclareSymbol("persistent user INDEX hud_iView;",              &hud_iView);
  _pShell->DeclareSymbol("persistent user INDEX hud_bShowOverlordInfo;",  &hud_bShowOverlordInfo);

//  _pShell->DeclareSymbol("persistent user CTString bot_strName1;",        &bot_strName1);
//  _pShell->DeclareSymbol("persistent user CTString bot_strName2;",        &bot_strName2);
//  _pShell->DeclareSymbol("persistent user CTString bot_strName3;",        &bot_strName3);
...
//  _pShell->DeclareSymbol("persistent user FLOAT gfx_bRenderFog;", &gfx_bRenderFog);
...
//*/
  ...
  void GetDetailStatsSP(CTString &strStats, INDEX iCoopType)
  {
    ...
    if (iCoopType<=1) {
      ...
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%d", TRANS("TOTAL SCORE"), m_psGameStats.ps_iScore));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%s", TRANS("DIFFICULTY"), GetDifficultyString()));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%s", TRANS("STARTED"), GetStatsRealWorldStarted()));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%s", TRANS("PLAYING TIME"), TimeToString(GetStatsRealWorldTime())));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%dX", TRANS("EXTRA ENEMY RATIO"), GetGameEnemyMultiplier()));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%d", TRANS("ENEMY MAX"), GetSP()->sp_iEnemyMax));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", TRANS("ENEMY SPEED"), GetSP()->sp_fEnemySpeedAdjuster*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", TRANS("PROJECTILE SPEED"), GetSP()->sp_fProjectileSpeed*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", TRANS("WEAPONS FIRE RATE"), GetSP()->sp_fWeaponsFireRate*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", TRANS("WEAPONS DAMAGE"), GetSP()->sp_fWeaponsDamage*100, "%"));
      strStats+="\n";
      strStats+=AlignString(CTString(0, "^cFFFFFF%s:^r\n^cFFFFFF%.0f%s", TRANS("PLAYER DAMAGE"), GetSP()->sp_fPlayerDamageAdjuster*100, "%"));
      strStats+="\n";
    ...
    /*if (iCoopType>=1) {
      if(m_strLevelStats!="") {
        //strStats += CTString("^cFFFFFF")+TRANS("Per level statistics") +"^r\n\n" + m_strLevelStats;
				strStats+=AlignString(CTString(0, "%s:^r\n%s", TRANS("Per level statistics"), m_strLevelStats));
      }
    }*/
  }
 ...
 ************************************************************/
  ...
  /*CEntity *GetViewEntity(void) {
    if (dwk_bBotView && GetFlags()&ENF_ALIVE) {
			...
			if (m_penBot1!=NULL && dwk_iBotView==1) {
				return ((CPlayerBot *)&*m_penBot1)->m_penView;
			} else if (m_penBot2!=NULL && dwk_iBotView==2) {
				return ((CPlayerBot *)&*m_penBot2)->m_penView;
			} else if (m_penBot3!=NULL && dwk_iBotView==3) {
				return ((CPlayerBot *)&*m_penBot3)->m_penView;
			} else if (m_penBot4!=NULL && dwk_iBotView==4) {
				return ((CPlayerBot *)&*m_penBot4)->m_penView;
			} else if (m_penBot5!=NULL && dwk_iBotView==5) {
				return ((CPlayerBot *)&*m_penBot5)->m_penView;
			} else if (m_penBot6!=NULL && dwk_iBotView==6) {
				return ((CPlayerBot *)&*m_penBot6)->m_penView;
			} else {
        // player eyes
        if (m_iViewState == PVT_PLAYEREYES) {
          return this;
    ...
    if( hud_bShowInfo) { 
      ...
      if( tmDelta<plr_tmSnoopingTime) {
        ...
        if (IsDerivedFromClass(pen->m_penTargeting, "Player")) {
          penHUDPlayer = (CPlayer*)&*pen->m_penTargeting; // bot snooping crash
        }
        bSnooping = TRUE;
      }

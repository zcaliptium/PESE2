    ...
    if( m_penRayHit!=NULL)
    {
      ...
      if( pen->GetFlags()&ENF_ALIVE)
      {
        ...
        if( IsOfClass( pen, "PlayerBot")) {
          ...
          // keep player name, mana and health for eventual printout or coloring
          m_strLastTarget.PrintF( "%s", pen->GetName());
          if( hud_bShowPlayerName) { m_tmLastTarget = tmNow+1.5f; }
        }
...
//    m_iAvailableWeapons &= ~WEAPONS_DISABLEDMASK;
  ...
  void AddDefaultAmmoForWeapon(INDEX iWeapon, FLOAT fMaxAmmoRatio)
  {
    ...
    switch (iWeapon) {
      ...
      // bullets
      case WEAPON_TOMMYGUN:
        iAmmoPicked = Max(50.0f, m_iMaxNukeBalls*fMaxAmmoRatio);
  ...
  BOOL ReceivePackAmmo(const CEntityEvent &ee)
  {
        ...
        (eapi.iNukeBalls>0 && m_iNukeBalls<m_iMaxNukeBalls))
    {
      ...
      if( eapi.iSniperBullets != 0) { strMessage.PrintF("%s %d %s,", strMessage, eapi.iSniperBullets, TRANS("Sniper bullets")); iAmmoTypes++; }
			if( eapi.iNukeBalls != 0)     { strMessage.PrintF("%s %d %s,", strMessage, eapi.iNukeBalls, TRANS("Da Bombs")); iAmmoTypes++; }
      ...
      /*if( m_penRayHit!=NULL) {
        ...
        if( pen->GetFlags()&ENF_ALIVE ) {
				  if ( IsOfClass(pen, "Player") ) {
					  ...
					  m_strLastTarget.PrintF(TRANS("Locked on %s"), ((CPlayer*)pen)->GetPlayerName());
					  m_tmLastTarget = _pTimer->CurrentTick()+2.0f;
				  } else if( IsDerivedFromClass(pen, "Enemy Base") ) {
					  // set up the target
					  m_penRocketLauncherHoming = pen;
					  m_strLastTarget.PrintF(TRANS("Locked on %s"), ((CEnemyBase*)pen)->GetIngameName());
					  m_tmLastTarget = _pTimer->CurrentTick()+2.0f;
				  } else {

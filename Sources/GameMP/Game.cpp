...
void CGame::InitInternal( void)
{
  ...
  _pShell->DeclareSymbol("persistent user INDEX gam_bExtraEnemiesPerPlayer;",&gam_bExtraEnemiesPerPlayer);
  _pShell->DeclareSymbol("persistent user FLOAT gam_fExtraEnemiesPerPlayerRatio;", &gam_fExtraEnemiesPerPlayerRatio );
  _pShell->DeclareSymbol("persistent user INDEX gam_iEnemyMultiplier;",      &gam_iEnemyMultiplier      );
  _pShell->DeclareSymbol("persistent user INDEX gam_iEnemyMax;",             &gam_iEnemyMax             );
  _pShell->DeclareSymbol("persistent user INDEX gam_bDamageSelf;",           &gam_bDamageSelf           );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fSpeedAdjuster;",        &gam_fSpeedAdjuster        );
  _pShell->DeclareSymbol("persistent user INDEX gam_bControlRunning;",       &gam_bControlRunning       );
  _pShell->DeclareSymbol("persistent user FLOAT gam_bBackwardsGame;",        &gam_bBackwardsGame        );
  _pShell->DeclareSymbol("persistent user INDEX gam_bGiveAllWeapons;",       &gam_bGiveAllWeapons       );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskChainsaw;",         &gam_bMaskChainsaw         );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskColt;",             &gam_bMaskColt             );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskSingleShotgun;",    &gam_bMaskSingleShotgun    );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskSniper;",           &gam_bMaskSniper           );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskFlamer;",           &gam_bMaskFlamer           );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskTommyGun;",         &gam_bMaskTommyGun         );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskRocketLauncher;",   &gam_bMaskRocketLauncher   );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskGrenadeLauncher;",  &gam_bMaskGrenadeLauncher  );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskMiniGun;",          &gam_bMaskMiniGun          );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskLaser;",            &gam_bMaskLaser            );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskCannon;",           &gam_bMaskCannon           );
  _pShell->DeclareSymbol("persistent user INDEX gam_bMaskTeleGun;",          &gam_bMaskTeleGun          );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fWeaponsFireRate;",      &gam_fWeaponsFireRate      );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fWeaponsDamage;",        &gam_fWeaponsDamage        );
	_pShell->DeclareSymbol("persistent user FLOAT gam_fProjectileSpeed;",      &gam_fProjectileSpeed      );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fGibLife;",              &gam_fGibLife              );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fEnemySpeedAdjuster;",   &gam_fEnemySpeedAdjuster   );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fPlayerDamageAdjuster;", &gam_fPlayerDamageAdjuster );
  _pShell->DeclareSymbol("persistent user INDEX gam_bUseBots;",              &gam_bUseBots              );
  _pShell->DeclareSymbol("persistent user INDEX gam_iBotsPerPlayer;",        &gam_iBotsPerPlayer        );
  _pShell->DeclareSymbol("persistent user FLOAT gam_fBotSkill;",             &gam_fBotSkill             );
  _pShell->DeclareSymbol("persistent user INDEX gam_iPostTGPLevel;",         &gam_iPostTGPLevel         );
	_pShell->DeclareSymbol("persistent user INDEX gam_bSkipLuxor;",						 &gam_bSkipLuxor            );
	_pShell->DeclareSymbol("persistent user INDEX gam_bSkipSewers;",					 &gam_bSkipSewers           );
	_pShell->DeclareSymbol("persistent user INDEX gam_bRandomLevels;",				 &gam_bRandomLevels				  );
  _pShell->DeclareSymbol("persistent user INDEX gam_bControlFlying;",        &gam_bControlFlying        );
  _pShell->DeclareSymbol("persistent user INDEX gam_bTestSpawnerBatteries;", &gam_bTestSpawnerBatteries );
	_pShell->DeclareSymbol("persistent user INDEX gam_bUsePEHealth;",          &gam_bUsePEHealth					);
	_pShell->DeclareSymbol("persistent user INDEX gam_iFPS;",									 &gam_iFPS									);
...
static void PrintStats( CDrawPort *pdpDrawPort)
{
  ...
  hud_iStats = Clamp( hud_iStats, 0L, 2L);
	...
	if (hud_iStats > 0) {
		...
		CTString strFPS = "?";
				 if( fFPS >= 20)   strFPS.PrintF( "%3.0f", fFPS);
...
void CGame::LCDSetDrawport(CDrawPort *pdp)
{
  ...
  if (pdp->dp_SizeIOverRasterSizeI==1.0f) {
	...
	//CPrintF("pdp->dp_SizeIOverRasterSizeI = %g\n", pdp->dp_SizeIOverRasterSizeI);
